-- Migration 017: reflect status_override in investments_effective view
-- Why: without this, reports that read the view see raw i.status even when a manual
-- override is set (e.g. a position written off privately but still listed Live upstream).
-- The override was only applied at WRITE time (upsert COALESCE); this makes it a
-- READ-time override too, so reports reflect the correct status immediately rather than
-- waiting for the next CSV re-import.
