-- Migration 067: make Direct effective values lifecycle-aware.
-- Why: a terminal lifecycle fact or legacy closed status must zero residual
-- unrealized value and classify matched cash distributions as realized proceeds.
...
SELECT
  ...
  -- Preserve the latest recorded snapshot as an auditable source fact.
  ...
  -- From reconciled cash-flow aggregates.
  ...
  i.asset_class,
  -- Lifecycle metadata is appended so existing view consumers retain their
  -- established column order and recorded valuation rows remain untouched.
