-- Migration 019: add best_unrealized_value to investments_effective view
-- Why: per-position IRR terminal value was inconsistent across reports.
-- portfolioSummary/thesis/performance use SUM(COALESCE(unrealized_value, invested))
-- (table column, locked→invested fallback). portfolioList read eff_unrealized_value
-- (valuation-snapshot only), so a LOCKED position with distributions computed IRR as
-- if the residual stake were worthless, and a snapshot-less table-valued position lost
-- its terminal entirely. portfolioDetail read i.unrealized_value (raw table) — a third
-- behavior. This column gives list and detail a single best-available unrealized source.
--
-- Semantics (mirrors best_total_value's precedence: snapshot, then table, then locked→cost):
--   COALESCE(lv.unrealized_value, i.unrealized_value, i.invested)
-- Write-offs encoded as unrealized_value = 0 stay at 0 (COALESCE keeps the 0); only
-- genuinely-locked positions (both snapshot and table NULL) fall back to invested.
-- Restated in full per migration conventions; new column appended at the END only.
